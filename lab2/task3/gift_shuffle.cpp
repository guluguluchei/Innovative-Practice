...
GIFT_Shuffle::GIFT_Shuffle() { memset(rk_, 0, sizeof(rk_)); }
GIFT_Shuffle::~GIFT_Shuffle() {}

void GIFT_Shuffle::set_key(const uint8_t* key) {
    uint16_t k[8];
    for (int i = 0; i < 8; i++) {
...
void GIFT_Shuffle::encrypt_block(const uint8_t* plaintext, uint8_t* ciphertext) const {
    uint8_t state[16];
    ...
    for (int r = 0; r < 40; r++) {
        ...
        // 2. AddRoundKey + AddRoundConstant (before PermBits, matching Basic/Bitslice)
        uint32_t rk_word = rk_[r];

        uint8_t rc = 0;
        int c = r;
        for (int i = 0; i < 6; i++) { rc = (rc << 1) | (c & 1); c >>= 1; }
        rc = ((rc << 1) | 1) & 0x3F;

        state[15] ^= 0x01;  // bit 127 = LSB of byte 15
        for (int i = 0; i < 6; i++) {
            if (rc & (1 << (5 - i))) {
                int pos = 127 - 4*i;
                state[pos/8] ^= (1 << (7 - (pos%8)));
            }
        }
        ...
        // 3. PermBits (after round key/constant, matching Basic/Bitslice)
        uint8_t bits[16];
        for (int i = 0; i < 16; i++) bits[i] = state[i];
        for (int i = 0; i < 16; i++) {
            state[i] = 0;
            for (int b = 0; b < 8; b++) {
                int src_bit = PERM[8*i + b];
                int src_byte = src_bit / 8;
                int src_shift = 7 - (src_bit % 8);
                if (bits[src_byte] & (1 << src_shift)) {
                    state[i] |= (1 << (7 - b));
                }
            }
        }
...
void GIFT_Shuffle::decrypt_block(const uint8_t* ciphertext, uint8_t* plaintext) const {
    uint8_t state[16];
    memcpy(state, ciphertext, 16);

    for (int r = 39; r >= 0; r--) {
        // 1. InvPermBits (inverse of PermBits, done first)
        uint8_t bits[16];
        for (int i = 0; i < 16; i++) bits[i] = state[i];
        for (int i = 0; i < 16; i++) {
            state[i] = 0;
            for (int b = 0; b < 8; b++) {
                int src_bit = PERM_INV[8*i + b];
                int src_byte = src_bit / 8;
                int src_shift = 7 - (src_bit % 8);
                if (bits[src_byte] & (1 << src_shift)) {
                    state[i] |= (1 << (7 - b));
        ...
        // 2. AddRoundConstant + AddRoundKey (XOR is self-inverse)
        uint8_t rc = 0;
        int c = r;
        for (int i = 0; i < 6; i++) { rc = (rc << 1) | (c & 1); c >>= 1; }
        rc = ((rc << 1) | 1) & 0x3F;
...
void GIFT_Shuffle::encrypt_4blocks(const uint8_t* in, uint8_t* out) const {
    for (int blk = 0; blk < 4; blk++) {
        encrypt_block(in + 16*blk, out + 16*blk);
    }
}

GIFT* GIFT::create(GIFTImpl impl) {
    switch (impl) {
        case GIFTImpl::BASIC:     return new GIFT_Basic();
        case GIFTImpl::BITSLICE:  return new GIFT_Bitslice();
        case GIFTImpl::SHUFFLE:   return new GIFT_Shuffle();
